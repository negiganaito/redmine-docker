...
class CmsRedirect
  ...
  def self.all
    RedmineupCms.redirects.map { |k, v| CmsRedirect.new(:source_path => k, :destination_path => v) }
  end
  ...
  def save
    ...
    redirects = Setting.plugin_redmineup_cms['redirects'].is_a?(Hash) ? Setting.plugin_redmineup_cms['redirects'] : {}
    redirects.merge!(source_path => destination_path)
    Setting.plugin_redmineup_cms = Setting.plugin_redmineup_cms.merge('redirects' => redirects)
  end
