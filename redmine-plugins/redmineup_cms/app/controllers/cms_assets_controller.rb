...
class CmsAssetsController < ApplicationController
  ...
  def thumbnail
    if (@attachment.token_auth(params[:token]) || @attachment.container_type == CmsSite.name || @attachment.visible?) && @attachment.thumbnailable? && tbnail = RedmineupCms::Thumbnail.generate(@attachment, params)
  ...
  def timelink
    if RedmineupCms::Cryptor.check_timestamp(@attachment.id, params[:timestamp], params[:key])
      send_file @attachment.diskfile, :filename => filename_for_content_disposition(@attachment.filename),
                                      :type => detect_content_type(@attachment),
                                      :disposition => (@attachment.image? ? 'inline' : 'attachment')
    else
