...
module CmsMenusHelper
  ...
  def change_menu_status_link(cms_menu)
    ...
    if cms_menu.active?
      link_to l(:button_lock), url.merge(:cms_menu => {:status_id => RedmineupCms::STATUS_LOCKED}), :method => :put, :class => 'icon icon-lock'
    else
      link_to l(:button_unlock), url.merge(:cms_menu => {:status_id => RedmineupCms::STATUS_ACTIVE}), :method => :put, :class => 'icon icon-unlock'
    end
  end
