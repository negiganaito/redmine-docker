  ...
  module CmsHelper
    ...
    def cached_render_part(part)
      if part.respond_to?(:is_cached) && part.is_cached?
        Rails.cache.fetch(part, :expires_in => RedmineupCms.cache_expires_in.minutes) {render_part(part)}
      else
        render_part(part)
      end
    end
    ...
    def render_liquid(content, part=nil)
      assigns = {}
      assigns['users'] = Redmineup::Liquid::UsersDrop.new(User.sorted)
      ...
      assigns['page'] = Redmineup::PageDrop.new(@cms_page) if @cms_page
      assigns['pages'] = Redmineup::PagesDrop.new(CmsPage.where(nil))
      assigns['params'] = self.params if self.respond_to?(:params)
      assigns['request'] = Redmineup::RequestDrop.new(request) if self.respond_to?(:request)
      ...
      registers["projects"] = Redmineup::Liquid::ProjectsDrop.new(Project.order(:name))
      begin
    ...
    def render_part(part)
      ...
      s = RedmineupCms::Textile::Formatter.new(s).to_html.html_safe if part.filter_id == "textile"
      s = Redcarpet::Markdown.new(Redcarpet::Render::HTML, :autolink => true, :tables => true).render(s).html_safe if part.filter_id == "markdown"
      s
    end

    def tags_options_for_select(klass, selected = nil)
      options_for_select([['']] + klass.available_tags.map{ |l| l.name }, selected)
    end
...
ActionView::Base.send :include, RedmineupCms::CmsHelper
::Liquid::Strainer.send :include, RedmineupCms::CmsHelper
