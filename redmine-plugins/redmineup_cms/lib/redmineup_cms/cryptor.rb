...
module RedmineupCms
  module Cryptor
    ...
    def self.secret_key
      Rails.version > '6.0' ? Rails.application.config.secret_key_base : Rails.application.config.secret_token
    end
    ...
    def self.generate_checksum(id, time)
      Digest::MD5.hexdigest("#{secret_key}/#{id}/#{time.utc.to_formatted_s(:number)}")
    end
  end
end
