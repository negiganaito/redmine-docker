  ...
  namespace :cms do
    ...
    task :migrate_settings => :environment do
      ...
      old_settings.each do |old_setting|
        if RedmineupCms.settings[:project][old_setting.project_id].blank?
          RedmineupCms.settings[:project][old_setting.project_id] = {}
        end
        RedmineupCms.settings[:project][old_setting.project_id][old_setting.name] = old_setting.value
      end
      RedmineupCms.save_settings
