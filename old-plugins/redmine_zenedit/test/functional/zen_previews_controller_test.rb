...
class ZenPreviewsControllerTest < ActionController::TestCase
  fixtures :projects, :issues, :issue_statuses, :issue_categories, :trackers, :projects_trackers,
           ...
           :enumerations, :journals, :journal_details, :email_addresses

  def setup
    @request.session[:user_id] = 1
  end

  def test_preview_formating
    compatible_xhr_request(:post, :preview, text: "Text with %{color: red}red% %{background:gray}color%")
    assert_match 'Click for edit', response.body
  end
end
