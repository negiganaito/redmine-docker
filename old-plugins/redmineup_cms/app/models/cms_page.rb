...
class CmsPage < defined?(ApplicationRecord) ? ApplicationRecord : ActiveRecord::Base
  include Redmine::SafeAttributes
  include RedmineupCms::Filterable
  include RedmineupCms::PageNestedSet
  ...
  scope :active, lambda { where(:status_id => RedmineupCms::STATUS_ACTIVE) }
  ...
  def self.visible_condition(user = User.current)
    ...
    cond = '(1=1) '
    cond << " AND (#{table_name}.status_id = #{RedmineupCms::STATUS_ACTIVE})" unless RedmineupCms.allow_edit?(user)
  ...
  def active?
    status_id == RedmineupCms::STATUS_ACTIVE
  end
  ...
  def page_fields
    @cms_page_fields ||= fields.inject({}) { |mem, var| mem[var.name] = var.content; mem }
  end
  ...
  def self.tags_cloud(options = {})
    ...
    join << "JOIN #{CmsPage.table_name} ON #{CmsPage.table_name}.id = #{Redmineup::Tagging.table_name}.taggable_id AND #{Redmineup::Tagging.table_name}.taggable_type =  '#{CmsPage.name}' "

    group_fields = ""
    group_fields << ", #{Redmineup::Tag.table_name}.created_on" if Redmineup::Tag.respond_to?(:created_on)
    group_fields << ", #{Redmineup::Tag.table_name}.updated_on" if Redmineup::Tag.respond_to?(:updated_on)
    ...
    scope = scope.group("#{Redmineup::Tag.table_name}.id, #{Redmineup::Tag.table_name}.name #{group_fields} HAVING COUNT(*) > 0")
  ...
  def process(listener, page_params = {})
    @cms_page_params = page_params.is_a?(Hash) ? page_params : {}
    @listener = listener
  ...
  def render
    if layout
      if is_cached? && RedmineupCms.layout_caching?
        Rails.cache.fetch("#{CmsSite.language}/#{layout.cache_key}/#{self.cache_key}/#{User.current.cache_key}", :expires_in => RedmineupCms.cache_expires_in.minutes) { render_object(layout) } 
      else
  ...
  def render_page
    if is_cached?
      Rails.cache.fetch(self, :expires_in => RedmineupCms.cache_expires_in.minutes) { render_page_with_content_parts(self) }
    else
  ...
  def render_part(part)
    ...
    if part.respond_to?(:is_cached) && part.is_cached?
      Rails.cache.fetch(part, :expires_in => RedmineupCms.cache_expires_in.minutes) { render_object(part) }
    else
      render_object(part)
    end
  end
  ...
  def initialize_context(page_listener)
    assigns = {}
    assigns['users'] = Redmineup::Liquid::UsersDrop.new(User.visible.sorted)
    assigns['projects'] = Redmineup::Liquid::ProjectsDrop.new(Project.visible.order(:name))
    assigns['newss'] = Redmineup::Liquid::NewssDrop.new(News.visible.order("#{News.table_name}.created_on"))
    assigns['current_user'] = Redmineup::Liquid::UserDrop.new(User.current)
    assigns['page'] = Redmineup::PageDrop.new(self)
    assigns['pages'] = Redmineup::PagesDrop.new(CmsPage.visible)
    ...
    assigns['site'] = Redmineup::SiteDrop.new
  ...
  def self.find_by_path(path)
    if path == '/' && RedmineupCms.landing_page
      CmsPage.find(RedmineupCms.landing_page)
    else
      RedmineupCms::PageFinder.find(path)
    end
  end
  ...
  def in_locale(page_locale = CmsSite.language)
    ...
    return CmsPage.find_by_path(page_locale) if id == RedmineupCms.landing_page
  ...
  def render_object(cms_object)
    ...
    text = RedmineupCms::HtmlCompressor::Compressor.new.compress(text) if false
    text.html_safe
  end
end
