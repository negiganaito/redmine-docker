...
class CmsImport
  ...
  def import
    data = safe_load_file(file.tempfile,
      permitted_classes: [Time, OpenStruct, Symbol, ActiveSupport::TimeZone, ActiveSupport::HashWithIndifferentAccess, ActiveSupport::TimeWithZone],
      aliases: true)
    import_object(data)
  end
  ...
  def safe_load_file(file_path, **options)
    if YAML.method(:safe_load).parameters.include?([:key, :permitted_classes]) && YAML.method(:load).parameters.include?([:key, :permitted_classes])
      YAML.load_file(file_path, **options)
    else
