...
class CmsMenu < defined?(ApplicationRecord) ? ApplicationRecord : ActiveRecord::Base
  include Redmine::SafeAttributes

  belongs_to :source, :polymorphic => true

  up_acts_as_list :scope => 'menu_type = \'#{menu_type}\' AND parent_id #{parent_id ? \'=\' + parent_id.to_s : \'IS NULL\'}'
  ...
  def self.menu_tree(menus, parent_id = nil, level = 0)
    tree = []
    menus.select { |menu| menu.parent_id == parent_id }.each do |menu|
      tree << [menu, level]
