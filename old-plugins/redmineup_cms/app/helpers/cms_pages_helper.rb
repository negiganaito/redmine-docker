...
module CmsPagesHelper
  ...
  def change_page_status_link(page)
    ...
    if page.active?
      link_to l(:button_lock), url.merge(:page => {:status_id => RedmineupCms::STATUS_LOCKED}), :method => :put, :class => 'icon icon-lock'
    else
      link_to l(:button_unlock), url.merge(:page => {:status_id => RedmineupCms::STATUS_ACTIVE}), :method => :put, :class => 'icon icon-unlock'
    end
  end
