...
class CmsVotesControllerTest < ActionController::TestCase
  ...
  def test_return_404_for_incorrect_params
    @request.session[:user_id] = 1
    compatible_request :get, :vote, :id => @cms_page.name, :vote => 'test'
    assert_response 404
  end
  ...
  def test_add_vote_for_page_for_user
    ...
    compatible_request :get, :vote, :id => @cms_page.name, :vote => 'up'
    assert_response 302
    assert_equal 1, @cms_page.votes_for.count
    assert_equal true, @cms_page.votes_for.last.vote_flag

    compatible_request :get, :vote, :id => @cms_page.name, :vote => 'down'
    assert_response 302
    assert_equal 1, @cms_page.votes_for.count
    assert_equal false, @cms_page.votes_for.last.vote_flag
  end
