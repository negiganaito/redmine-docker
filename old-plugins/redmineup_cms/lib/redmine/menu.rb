...
Redmine::MenuManager.map :top_menu do |menu|
    ...
    menu.push :projects, { :controller => 'projects', :action => 'index' }, :caption => :label_project_plural, :if => Proc.new { Setting.plugin_redmineup_cms["show_projects"].to_i > 0 }, :first => true
    menu.push :home, { :controller => 'welcome', :action => 'index' }, :if => Proc.new { Setting.plugin_redmineup_cms["show_home"].to_i > 0 }, :first => true
    menu.push :help, Redmine::Info.help_url, :last => true, :if => Proc.new { Setting.plugin_redmineup_cms["show_help"].to_i > 0 }
end
...
Redmine::MenuManager.map :project_menu do |menu|
  menu.push :activity, { :controller => 'activities', :action => 'index' }, :if => Proc.new{|p| !p.module_enabled?(:cms) || RedmineupCms.get_project_settings("project_tab_show_activity", p.id).to_i > 0 }, :after => :new_object
  menu.push :overview, { :controller => 'projects', :action => 'show' }, :if => Proc.new{|p| !p.module_enabled?(:cms) || RedmineupCms.get_project_settings("landing_page", p.id).blank? }, :after => :new_object
  ...
  10.downto(1) do |index|
    ...
    menu.push tab, {:controller => 'project_tabs', :action => 'show', :tab => index},
                             :param => :project_id,
                             :after => :new_object,
                             :caption => Proc.new{|p| RedmineupCms.get_project_settings("project_tab_#{index}_caption", p.id) || tab.to_s },
                             :if => Proc.new{|p| !RedmineupCms.get_project_settings("project_tab_#{index}_caption", p.id).blank? }
  end
  menu.push :project_tab_last, {:controller => 'project_tabs', :action => 'show', :tab => "last"},
                           :param => :project_id,
                           :last => true,
                           :caption => Proc.new{|p| RedmineupCms.get_project_settings("project_tab_last_caption", p.id) || tab.to_s },
                           :if => Proc.new{|p| !RedmineupCms.get_project_settings("project_tab_last_caption", p.id).blank? }
