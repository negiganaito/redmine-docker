...
module Redmineup
  class PartsDrop < Redmineup::BaseDrop
    def all
    ...
    def [](key)
      if part = @_source.detect { |p| p.name == key   }
        PartDrop.new part
      else
        super(key)
      end
  ...
  class PartDrop < Redmineup::BaseDrop
    ...
    def page
      @_source ||= Redmineup::PageDrop.new(@_source.page) if @_source.page
    end
  end
end
