...
module Redmineup
  class PagesDrop < Redmineup::BaseDrop
    ...
    def [](key)
      if page = @_source.detect { |p| p.name == key   }
        PageDrop.new page
      else
        super(key)
      end
  ...
  class PageDrop < Redmineup::BaseDrop
    delegate :id,
            :name,
            :slug,
            :path,
    ...
    def url(options = {})
      ...
      page_path = @_source.in_locale(page_locale || CmsSite.language).try(:path) if RedmineupCms.use_localization? && localized
    ...
    def parts
      @cms_parts ||= Redmineup::PartsDrop.new @_source.parts
    end
    ...
    def author
      @author ||= Redmineup::Liquid::UserDrop.new(@_source.author) if @_source.author
    end
    ...
    def attachments
      ...
      @attachments = {}
      @_source.attachments.each { |f| @attachments[f.filename] = Redmineup::CmsAttachmentDrop.new f }
      @attachments
    end
