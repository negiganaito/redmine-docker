      ...
      module Render
        ...
        class PageContent < ::Liquid::Tag
          def render(context)
            ...
            raise ::Liquid::Error, "Page not found in Liquid context" if page.blank?
            context.stack do
        ...
        class PartContent < ::Liquid::Tag
          ...
          def initialize(tag_name, markup, options)
            super
            if markup =~ Syntax
              @cms_part_var = ::Liquid::Variable.new($1, options)
            else
              raise ::Liquid::Error, "Syntax error {% part_content 'part_name' %}"
            end
          end

          def render(context)
            part_name = @cms_part_var.render(context)
            page = context.registers[:page]
            ...
            return '' unless part.active?
            context.stack do
              page.render_part(part)
            end
          end
        end
        ...
        # Render 'news' snippet:
        #
        #   {% render_snippet 'news', param1: 'param_value', param2: page  %}
        #
        class RenderSnippet < ::Liquid::Tag
          ...
          def initialize(tag_name, markup, options)
            super
            raise ::Liquid::Error, "Syntax error {% render_snippet 'snippet_name', param1: 'param_value', param1: pages['main'] %}" unless markup =~ Syntax

            @snippet_name_expr = ::Liquid::Expression.parse($1)
          ...
          def render(context)
            page = context.registers[:page]
            snippet_name = @snippet_name_expr
            return context["#{snippet_name}_snippet_content"] unless page
            page.context = context
            snippet = CmsSnippet.where(:name => snippet_name).first
            raise ::Liquid::Error, """render_snippet '#{snippet_name}'"": no such snippet '#{snippet_name}'" if snippet.blank?
        ...
        class RenderFile < ::Liquid::Tag
          ...
          def initialize(tag_name, markup, context)
            super
            if markup =~ Syntax
              @param_var = ::Liquid::Variable.new($1, context)
            else
              raise ::Liquid::Error, "Syntax error {% render_file 'attachment_name' %}"
            end
          end
        ...
        class IfPartContent < ::Liquid::Block
          ...
          def initialize(tag_name, markup, context)
            ...
            if markup =~ Syntax
              ...
              raise ::Liquid::Error, "[if_part_content] Syntax error {% if_part_content 'part_name' %} {% endif_part_content %}"
            end
          end
